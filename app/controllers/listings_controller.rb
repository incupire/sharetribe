class ListingsController < ApplicationController
	...
	skip_before_action :verify_authenticity_token, :only => [:close, :update, :follow, :unfollow, :autocomplete_person_emails]
	...
	def show
		if @listing.is_private && @listing.author_id != @current_user.id && @listing.buyer_id != @current_user.id
			flash[:error] = "You are not authorized to view this listing."
	...
	def create
		...
		ActiveRecord::Base.transaction do
			...
			if @listing.save
				...
				if params[:listing][:buyer_id].present?
					#new custom offer listing
					buyer = Person.joins(:emails).where(emails: {address: params[:listing][:buyer_id]}).first
					unless buyer.present?
						flash[:error] = "Invalid buyer!"
						redirect_to new_listing_path
						return
					else
						if @listing.update(is_private: true, buyer_id: buyer.id)
							...
							conversation.build_starter_participation(@listing.author)
							conversation.build_participation(buyer)
							conversation.save
	...
	def validate_listing_buyer
		buyer = Person.joins(:emails).where(emails: {address: params[:listing][:buyer_id]}).first
		respond_to do |format|
			format.json { render :json => buyer.present? ? true : false }
		end
	end

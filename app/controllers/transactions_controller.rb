  ...
  def price_break_down_locals(tx, conversation)
    if tx.payment_process == :none && tx.unit_price.cents == 0 || conversation.starting_page == Conversation::LISTING
      ...
      TransactionViewUtils.price_break_down_locals({
        ...
        total: (tx.payment_gateway.eql?(:coupon_pay) && tx.starter.eql?(current_user)) ? Maybe(tx.payment_total).or_else(payment[:total_price]) + tx.commission : Maybe(tx.payment_total).or_else(payment[:total_price]),
        seller_gets: tx.payment_gateway.eql?(:coupon_pay) ? (Maybe(tx.payment_total).or_else(payment[:total_price])) : (Maybe(tx.payment_total).or_else(payment[:total_price]) - tx.commission),
        fee: tx.payment_gateway.eql?(:coupon_pay) ? Money.new(0, tx.item_total.currency) : tx.commission,

class TransactionPreauthorizedJob < Struct.new(:transaction_id)
  ...
  def perform
    transaction = Transaction.find(transaction_id)
    transaction_url = Rails.application.routes.url_helpers.person_transaction_url(person_id: transaction.author.id, id: transaction.id, :host => if Rails.env.eql?("development") then "lvh.me:3000" elsif Rails.env.eql?("staging") then "staging.avontage.com" else "staging.avontage.com" end )
